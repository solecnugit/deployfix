...
impl<Tz> DeepMerge for chrono::DateTime<Tz>
where
    Tz: chrono::TimeZone,
{
    default_overwriting_impl! {}
...
impl<T> DeepMerge for Spanned<T>
where
    T: DeepMerge,
{
    fn merge_from(&mut self, other: Self) {
...
pub mod strategies {
    ...
    pub mod list {
        ...
        pub fn atomic<V>(current: &mut V, new: V)
        where
            V: AsOptVec,
        {
            ...
            V: AsOptVec,
        {
            if let Some(current) = current.as_mut_opt() {
                for new_item in new.into_opt().into_iter().flatten() {
                    if let Some(current_item) = current.iter_mut().find(|current_item| {
                        key_comparators
                            .iter()
                            .all(|&f| f(&**current_item, &new_item))
                    }) {
        ...
        pub fn set<V>(current: &mut V, new: V)
        where
            V: AsOptVec,
            V::Item: PartialEq,
        {
    ...
    pub mod map {
        mod private {
            ...
            impl<K, V> AsOptMap for Spanned<BTreeMap<K, V>> {
                type Key = K;
                type Value = V;
                ...
                fn into_opt(self) -> Option<BTreeMap<K, V>> {
                    Some(self.value)
                }
            }

            impl<K, V> AsOptMap for Option<Spanned<BTreeMap<K, V>>> {
                type Key = K;
                type Value = V;
                ...
                for (k, new_v) in new.into_opt().into_iter().flatten() {
                    match current.entry(k) {
                        ...
                        std::collections::btree_map::Entry::Occupied(entry) => {
                            merge_value(entry.into_mut(), new_v)
                        }
                    }
                }
            } else {
